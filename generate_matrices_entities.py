...
IN_NAME = 'pickles/rel_v2_entities_df.tar.gz' # 'pickles/philoso_named_entity_df.tar.gz'
OUT_PREFIX = 'rel_v2'
...
del sparse
dtm.to_pickle(f'pickles/dtm_{OUT_PREFIX}_BOW_entities.tar.gz')

ttm = dtm.dot(dtm.transpose())
ttm.to_pickle(f'pickles/ttm_{OUT_PREFIX}_BOW_entities.tar.gz')
...
dtm.index = philoso_corpus.dictionary.values()
del sparse
dtm.to_pickle(f'pickles/dtm_{OUT_PREFIX}_TF-IDF_entities.tar.gz')

ttm = dtm.dot(dtm.transpose())
ttm.to_pickle(f'pickles/ttm_{OUT_PREFIX}_TF-IDF_entities.tar.gz')
